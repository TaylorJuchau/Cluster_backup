...
from phangs_data_access import helper_func, phot_tools
...
pixel_size_wfc3_uvis = 0.04


wfc3uv_band_list = ['F225W', 'F275W', 'F336W', 'F390W', 'F438W', 'F467M', 'F555W', 'F606W', 'F621M', 'F775W', 'F814W', 'F850L']

psf_dict =

for band in wfc3uv_band_list:

    hdu = fits.open(f'data/WFC3UV/STDPSF_WFC3UV_{band}.fits')
    data = hdu[0].data
    mean_psf = np.mean(data, axis=0)

    print(mean_psf.shape)
    ...
    print(central_x_pos, central_y_pos, max_rad)
    # plt.imshow(np.log10(mean_psf))
    # plt.scatter(central_x_pos, central_y_pos)
    # plt.show()
    radius, profile, profile_error = phot_tools.PhotTools.get_rad_profile(data=mean_psf, x_pos=central_x_pos,
                                                                          y_pos=central_y_pos, max_rad=max_rad,
                                                                          err=None, method='exact')
...
if not os.path.isdir('data'):
    os.makedirs('data')
helper_func.FileTools.download_file(file_path=file_path_uvis1, url=url_uvis1)
helper_func.FileTools.download_file(file_path=file_path_uvis2, url=url_uvis2)
...
helper_func.FileTools.download_file(file_path=file_path_acs_wfc1_url, url=acs_wfc1_url)
